Surfaces.prototype.spheraZACHET = (count = 30, R = 6, point = new Point(0, 0, 0), color = 'ff0000', animation) => {
    let points = [];
    let edges = [];
    let polygons = [];
    ...
    for (let i = 0; i < points.length; i++) {
        if (i + 1 + count < points.length && (i + 1) % count !== 0) {
            polygons.push(new Polygon([i, i + 1, i + 1 + count, i + count], '#008080'));
        } else if ((i + count) < points.length && (i + 1) % count === 0) {
            polygons.push(new Polygon([i, i + 1 - count, i + 1, i + count], '#800000'));
        }
    }
